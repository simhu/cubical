...
inhRec b p f v@(Kan i (VInh a) us u) = comp Pos j ffillu us' u'
  where j    = fresh (b,p,f,v)
        usij = us `swap` (i,j)
        us'  = Map.mapWithKey irec usij
        u'   = irec (i ~> 0) u
        ffillu     = app f (fill Pos j (VInh a) usij u)
        irec alpha = inhRec (b `face` alpha)
                       (p `face` alpha) (f `face` alpha)
