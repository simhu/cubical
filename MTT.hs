...
upds env []          = env
upds env ((x,v):xvs) = upds xvs (Pair env x v)
...
getE x (Pair s _ _)                = getE x s
getE x r@(PDef es r1) = getE x (upds r1 (evals (map fst es) r))

addC :: Cont -> Tele -> Env -> [(String,Val)] -> Cont
addC gam _      _  []     = gam
addC gam (a:as) nu ((x,u):xus) = addC (eval a nu:gam) as (Pair nu x u) xus
...
checkTUs k rho gam ((x,a):xas) = do
  check k rho gam U a
  checkTUs (k+1) (Pair rho x (Var k)) ((x,eval a rho):gam) xas

--addC :: Cont -> [Exp] -> Env -> [(String,Val)] -> Cont
-- What does this do?
checkBranch :: Int -> Env -> Cont -> [(Binder,Exp)] -> Env -> Val -> String -> [Binder] -> Exp -> Error ()
checkBranch k rho gam xas nu f c xs e = do
  let l  = length xas
  let us = map Var [k..k+l-1]
  check (k+l) (upds rho (zip xs us)) (addC gam (map snd xas) nu (zip xs us)) (app f (Con c us)) e
...
checkI k rho gam e = case e of
  ...
  Def t d@(xas,xes) -> -- trace ("checking definition " ++ show str)
    (do
      checkD k rho gam d
      let rho1 = PDef d rho
      checkI k rho1 (addC gam (map snd xas) rho (evals xes rho1)) t)
  PN _ a -> do
    checkT k rho gam a          -- ??
    return (eval a rho)
...
checks :: Int -> Env -> Cont -> [(Binder,Exp)] -> Env -> [(Binder,Exp)] -> Error ()
checks _ _   _    []    _  []     = return ()
checks k rho gam ((x,a):xas) nu ((_,e) :xes) = do
  -- trace ("checking " ++ show e ++ "\nof type " ++ show a
  --        ++ "\nin " ++ show rho ++ "\n")
  check k rho gam (eval a nu) e
  checks k rho gam xas (Pair nu x (eval e rho)) xes
...
reifyExp _ Top                 = Top
reifyExp _ U                   = U
reifyExp k (Comp (Lam x t) r)  = Lam (genName k) $ reifyExp (k+1) (eval t (Pair r x (Var k)))
reifyExp k (Var l)             = Ref $ genName (k-l-1) -- a new name
